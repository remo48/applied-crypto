...
def _decrypt_byte(oracle: Oracle, known_bytes: bytes, ciphertext: bytes, block_length: int = 16):
    j = block_length - len(known_bytes) - 1

    for i in range(0x100):
        guess = i.to_bytes(1, 'big')
        padding = bytes(j) + known_bytes + guess + bytes(j)
        result = oracle.ecb_enc(padding, ciphertext)
        if result[:16] == result[16:32]:
            return guess
    return None
...
def attack(oracle: Oracle, ciphertext: bytes, block_length: int = 16):
    ...
    for j in range(0, len(ciphertext), 16):
        for i in range(16):
            guess = _decrypt_byte(oracle, result[j:], ciphertext[j:])
            if not guess:
                break
            result += guess
    print(result)
